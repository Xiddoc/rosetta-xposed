 ...
 * and [Targets] feeds that string verbatim into `Class.forName(...)` followed
 * by `setAccessible(true)`. A malicious or simply wrong map could therefore
 * redirect a hook at a sensitive framework class — `java.lang.Runtime`,
 * `android.app.*`, a `dagger.internal.Provider`, etc. — and the binding would
 * happily load it and make its members accessible.
 *
 * MITIGATION. A *target* (the FQN passed to `Class.forName`:
 * `ResolvedClass.obfName` and `ResolvedMethod`/`ResolvedField.className`) is
 * confined to the app's own / package-local namespace, with an explicit
 * escape-hatch allowlist for legitimate framework hooks. Anything else is
 * rejected fail-closed — the binding THROWS
 * [io.github.xiddoc.rosetta.core.TargetPolicyException] BEFORE the class load
 * and before `setAccessible`. There is no warn-and-proceed mode (strict only).
 *
 * The Frida-side twin mirrors these exact semantics (same decision order, same
 * [DEFAULT_DENY_PREFIXES]); keep the two in lockstep.
 */
...
public data class TargetPolicy(
    ...
    val appNamespaceLabels: Int = 2,
) {
    /** The effective denylist after applying [mergeDenylist]. */
    internal val effectiveDenyPrefixes: List<String> =
 ...
 * reflection so it is unit-testable in isolation; [Targets] is the only
 * runtime caller.
 */
